...
用户要装"角色声线"时，你必须把素材的来历说清楚，而且**不能替权利人表态**。

- **代码是 MIT，素材不是。** `voice/LICENSE.txt` 是**素材声明，不是许可证**。
- **参考音是权利人的游戏原声**，不是本项目录的，**不在 MIT 覆盖范围内**。
- 角色与声音的权利属于权利人（本项目自带的那条属于米哈游）。**本项目不持有这些权利，因此无法授予任何许可。**
- 所以"仅限学习研究、不得商用"要讲成**维护者的意愿/请求**，不要说成"本插件授权你非商用"或"本项目禁止你商用"—— **我们无权授予，也无权禁止。**
- 商用需要权利人的许可，**这份许可只有权利人能给**。

装那类声线前，先让用户读 `voice/LICENSE.txt`。**不要**自作主张补一句"放心用"或"我保证合规"。
