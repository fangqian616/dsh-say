    ...
    options: [
      {
        ...
        description: '约 1.1 GB，比官方整合包省将近 5.5 GB；需要 Python（脚本会自己建环境）。速度比整合包慢一些，但远快于实时。',
      },
      {
...
export async function onboard(options) {
  ...
  if (assertsOwnEngine && !engineAvailable) {
    ...
    todos.push(
      [
        '用户选择了 ONNX 版（不需要 GPT-SoVITS，约 1.1 GB）',
        '这一步要主动做完，并如实转述结果：',
  ...
  if (engineAvailable !== true && onnxAvailable !== true && (wantsVoice || hasVoicePack) && !/ONNX/.test(backendChoice) && !assertsOwnEngine) {
    ...
    todos.push(
      [
        ...
        '明确告诉用户这一步，不要说"已经装好了"',
        '然后二选一，把体积和前置条件都讲清楚：',
        `① ONNX 版：\`node scripts/install-onnx.mjs\`，约 1.1 GB，需要 Python，比整合包慢一些但远快于实时`,
