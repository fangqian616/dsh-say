...
export function summarizeLocally(text, budget = 110) {
  ...
  // A dangling back-reference is removed before anything else, because it
  // changes which sentences remain and therefore whether the enumeration is
  // still complete.
  ...
  return {
    text: reconciled !== null ? normalizeForSpeech(reconciled) : joined,
