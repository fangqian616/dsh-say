...
const NOISE_PATTERNS = [
  ...
  [/(?<=\s|^)[*_](?=\S)([^*_\n]*?\S)[*_](?=\s|$|[。！？，,.;；])/g, '$1'],
  [/\*/g, ''],
  // A hash or digest is never worth hearing. A synthesizer reading one aloud
  // produces a string of letter names ("四 f 七九 ae 三") that a listener cannot
  // hold or use, and it crowds out a sentence that carried an outcome.
  //
  // It matches only where the run is *known* to be a hash — bounded by
  // punctuation, or sitting where a filename carries its content tag — because
  // a bare `[0-9a-f]{7,}` also matches ordinary numbers. A version tag or a
  // count is pronounceable and is often the whole point ("升级到 3.18.2").
  //
  // Replacing with '' rather than a placeholder keeps the sentence readable;
  // the leftover punctuation is collapsed right after, so "app.<hash>.js"
  // becomes "app.js" and "commit <hash>。" becomes "commit。".
  [/(?<=[^0-9a-zA-Z]|^)(?=[0-9a-f]*[a-f])[0-9a-f]{7,40}(?=[^0-9a-zA-Z]|$)/g, ''],
  [/(?<=\.)[0-9a-f]{7,40}(?=\.[a-z]{1,5}\b)/g, ''],
]
...
export function stripForSpeech(text) {
  ...
  return out
    .replace(/[ \t]+/g, ' ')
    // Two dots appear only where a stripped hash sat between a name and its
    // extension; collapsing them keeps "app.<hash>.js" from reading as a pause.
    .replace(/\.{2,}/g, '.')
    .replace(/[ \t]+([。！？，,.;；])/g, '$1')
    .replace(/。{2,}/g, '。')
    .replace(/\n{2,}/g, '\n')
...
export async function compressWithSubagent(options) {
  ...
  try {
    run = await subagents.start(names[0], {
      label: 'dsh-say report compression',
