 ...
 *
 * @module dsh-voice/lib/engines/gptsovits
 */
...
"""dsh-voice helper: synthesize one line with GPT-SoVITS.

Written by dsh-voice on demand; safe to delete. Reads a JSON request from
...
export async function discover(config, options = {}) {
  ...
  discoveryCache = {
    ...
    mode: 'none',
    reason:
      'no GPT-SoVITS checkout found. Set DSH_VOICE_ENGINE_ROOT, pass engines.gptSovits.engineRoot, or use the built-in engine instead. See docs/ENGINES.md.',
  }
  return discoveryCache
}
...
export async function synthesize(request) {
  ...
  const requestPath = join(workDir(), `dsh-voice-request-${Date.now()}-${process.pid}.json`)
  const payload = {
    mode: 'local',
