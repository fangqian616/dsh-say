...
/**
 * Where a GPT-SoVITS checkout might be, most specific first.
 *
 * There is no single install location: the official Windows package is a 7z that
 * people unzip wherever they have room, which is often a drive root on a secondary
 * disk (`E:\GPT-SoVITS-main`). Hard-coding `C:\` and `D:\` therefore missed the
 * common case entirely, and the user got "no GPT-SoVITS checkout found" for an
 * engine that was sitting in plain sight.
 *
 * Scanning one level of every drive costs a few directory reads and is only done
 * once per discovery, so it is cheaper than asking the user for a path.
 */
function rootCandidates(configured) {
  ...
  candidates.push(join(homedir(), 'gpt-sovits'))
  for (const found of scanDrivesForEngine()) candidates.push(found)
  return candidates
}

/**
 * Directories named like a GPT-SoVITS checkout at the root of each drive.
 *
 * Windows-only, and deliberately shallow: an unzip target is one level down, while
 * a recursive search over several disks would stall the first tool call.
 */
function scanDrivesForEngine() {
  ...
  for (let code = 65; code <= 90; code += 1) {
    ...
    try {
      ...
      continue // No such drive, or not readable. Either way, nothing to find.
    }
    for (const entry of entries) {
      if (!entry.isDirectory()) continue
      if (/^gpt[-_]?sovits/i.test(entry.name)) found.push(join(drive, entry.name))
    }
  }
  return found
}
...
/**
 * Accept either the whole plugin config or just the `engines.gptSovits` block.
 *
 * Callers do both, and both are reasonable: `index.js` has no other use for the
 * rest of the config, while the tool layer must pass the whole thing because it
 * also needs `speed` and `textLang` from the top level.
 *
 * Reading engine options without this normalization does not fail - it silently
 * falls back to the defaults, because every lookup is `config.x || default`. That
 * is how a configured `engineRoot`, `device`, and `isHalf` went unused: the drive
 * scan happened to find this machine's engine, so nobody noticed.
 */
export function engineOptions(raw) {
  const full = raw || {}
  const block = full.engines?.gptSovits
  if (!block) return full
  // Top-level keys survive, the sub-block wins where both speak: it is the more
  // specific statement of the same option.
  return { ...full, ...block }
}
 ...
 *
 * @param {object} rawConfig resolved plugin configuration, or its gptSovits block
