 ...
 *
 * This is the engine that makes dsh-say install-and-run: no model download,
 ...
 *
 * @module dsh-say/lib/engines/builtin
 */
...
const SPEAK_PS1 = [
  ...
  '  $candidates = @()',
  '  if ($env:TEMP) { $candidates += (Join-Path $env:TEMP "dsh-say") }',
  '  if ($env:SystemRoot) { $candidates += (Join-Path $env:SystemRoot "Temp\\dsh-say") }',
  '  $candidates += "C:\\Windows\\Temp\\dsh-say"',
  '  try { $candidates += (Join-Path ([IO.Path]::GetTempPath()) "dsh-say") } catch { }',
  ...
  '    $target = Join-Path (New-SafeTemp) ("dsh-say-" + [Guid]::NewGuid().ToString("N") + ".wav")',
  '  }',
  '  $synth = New-Object System.Speech.Synthesis.SpeechSynthesizer',
  '  $synth.Volume = 100',
  '  $synth.Rate = $Rate',
...
function scriptFile(tag, source) {
  const path = join(workDir(), `dsh-say-${tag}.ps1`)
...
export async function synthesize(request) {
  ...
  const textPath = join(workDir(), `dsh-say-text-${Date.now()}-${process.pid}.txt`)
