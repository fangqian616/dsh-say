...
export async function voices(args, ctx) {
  ...
  if (action === 'add') {
    ...
    writeFileSync(
      join(dir, 'pack.json'),
      `${JSON.stringify({ name, version, gpt, sovits, license: args.license || '' }, null, 2)}\n`,
      'utf8',
    )
      ...
      parameters: {
        ...
        properties: {
          ...
          engine: {
            ...
            description:
              'Force one engine. Use "builtin" for English speech: its OS voices are native per language, while ' +
              'the gpt-sovits engine is cross-lingual and reads English with the voice pack\'s accent.',
          },
          speed: { type: 'number', description: 'Speaking rate, 0.5 to 2.0. Defaults to the configured value.' },
          textLang: {
            type: 'string',
            description: 'Language of the text, e.g. zh or en. Set it when the text is not the pack\'s language.',
          },
      ...
      parameters: {
        ...
        properties: {
          ...
          engine: {
            type: 'string',
            enum: ['auto', 'builtin', 'gpt-sovits'],
            description: 'Force one engine. Use "builtin" for English: its OS voices are native per language.',
          },
          textLang: { type: 'string', description: 'Language of the report, e.g. zh or en.' },
    ...
    },
    {
      ...
      parameters: {
        ...
        properties: {
          ...
          license: { type: 'string', description: 'add: a note about the terms of the material, for your own records.' },
        },
        required: ['action'],
      },
      output: {
