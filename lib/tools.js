...
import genie from './engines/genie.js'
...
/**
 * The notice written into a pack that is registered without one.
 *
 * This is the wording used across the project for the bundled voice: learning and
 * exchange only, no commercial use, removed on request. It states the user's
 * obligations rather than claiming a grant, because a maintainer of a cloned
 * character voice holds nothing to grant.
 */
 ...
 *
 * `auto` prefers a character voice over the OS voices, and among the two engines
 * that can produce one it prefers the GPT-SoVITS the user already has: that choice
 * costs no download at all, while the ONNX engine is the one you install when you
 * have no engine yet. Picking ONNX first would quietly push an 800 MB install on
 * someone who already solved this problem.
 */
export async function selectEngine(ctx, requested) {
  ...
  if (wanted === 'gpt-sovits') {
    const status = await gptsovits.discover(config.engines?.gptSovits || config)
    if (!status.available) return { ok: false, reason: status.reason }
    return { ok: true, engine: gptsovits, id: 'gpt-sovits', status }
  }

  if (wanted === 'onnx') {
    const status = await genie.discover(config.engines?.onnx || config)
    if (!status.available) return { ok: false, reason: status.reason }
    return { ok: true, engine: genie, id: 'onnx', status }
  }
  ...
  if (healthy.length > 0) {
    ...
    // No GPT-SoVITS checkout, but a pack exists and the ONNX engine can read it.
    const onnxStatus = await genie.discover(config.engines?.onnx || config)
    if (onnxStatus.available) return { ok: true, engine: genie, id: 'onnx', status: onnxStatus }
  }
  ...
  return {
    ...
    reason:
      ...
      `For character voices install GPT-SoVITS, or the ONNX engine, and register a voice pack in ${config.voicesDir}.`,
  }
}
...
/** `tts_engines` — what can speak right now, plus the active report persona. */
export async function engines(_args, ctx) {
  const builtinStatus = await builtin.probe()
  const gptStatus = await gptsovits.discover(ctx.config)
  const onnxStatus = await genie.discover(ctx.config)
  const packs = listVoicePacks({ voicesDir: ctx.config.voicesDir })
  const persona = readPersona(ctx)
  ...
  return {
    ...
    onnx: {
      available: onnxStatus.available === true,
      python: onnxStatus.python || '',
      dataDir: onnxStatus.dataDir || '',
      reason: onnxStatus.reason || '',
      // What onnxruntime has compiled in. `provider` is what a loaded session
      // actually got, and the two disagree exactly when CUDA was requested and
      // silently refused - which is the case worth telling the user about.
      availableProviders: onnxStatus.availableProviders || [],
      requestedDevice: onnxStatus.device || 'auto',
      // Chinese and English always work; Japanese is a deliberate extra install.
      japanese: onnxStatus.japanese || 'unknown',
    },
    voicePacks: packs.map((pack) => ({
      name: pack.name,
...
export async function voices(args, ctx) {
  ...
  if (action === 'add') {
    ...
    const onnx = typeof args.onnx === 'string' ? args.onnx.trim() : ''
    // Either pair works: PyTorch weights for GPT-SoVITS, an ONNX model directory
    // for the ONNX engine, or both for a pack that can be spoken either way.
    // Requiring the PyTorch pair would make the ONNX-only pack - which is exactly
    // what someone with no engine gets - impossible to register.
    if (!refAudio || (!onnx && (!gpt || !sovits))) {
      return failure('add needs refAudio plus either gpt and sovits (PyTorch) or onnx (an ONNX model directory)')
    }
    if (onnx && !existsSync(onnx)) {
      return failure(`the ONNX model directory does not exist: ${onnx}`)
    }
    ...
    // Stored absolute. The model lives where it was converted, which is usually
    // outside the pack, and copying a few hundred megabytes to satisfy a path
    // convention is not a trade worth making.
    if (onnx) entry.onnx = isAbsolute(onnx) ? onnx : resolve(onnx)
    // `notice` records the material's terms; a pack registered without one gets
    // the project's standard learning-use notice. `license` is still read for
    // packs written by older versions.
    entry.notice = args.notice || args.license || STANDARD_NOTICE
    writeFileSync(join(dir, 'pack.json'), `${JSON.stringify(entry, null, 2)}\n`, 'utf8')
  ...
  return [
    {
      ...
      parameters: {
        ...
        properties: {
          ...
          engine: {
            type: 'string',
            enum: ['auto', 'builtin', 'gpt-sovits', 'onnx'],
    ...
    },
    {
      name: 'tts_engines',
      description:
        'Report which text-to-speech engines dsh-say can use right now: the built-in OS voices, a local GPT-SoVITS ' +
        'checkout (with the model weights it found), the ONNX engine (with the execution provider it actually got), ' +
        'or a remote GPT-SoVITS server. Also returns the active report persona (the soul/ notes and the voice pack ' +
        'it selects). Use it to diagnose a silent setup, and before speaking a report so the wording matches the persona.',
      ...
      output: {
        ...
        render: (_args, value) => {
          ...
          const onnx = result.onnx || {}
          lines.push(onnx.available
            ? `onnx: available (${onnx.python}${onnx.dataDir ? `, data: ${onnx.dataDir}` : ''}, ` +
              `requested: ${onnx.requestedDevice}, build: ${(onnx.availableProviders || []).join('/') || 'unknown'})`
            : `onnx: unavailable — ${onnx.reason || 'not found'}`)
          if (onnx.available && onnx.japanese && onnx.japanese !== 'installed') {
            lines.push(`  japanese: ${onnx.japanese === 'stand-in' ? 'not installed (run install-onnx.mjs --with-japanese)' : 'missing'}`)
          }
          lines.push(`voice packs: ${(result.voicePacks || []).length} in ${result.voicesDir || ''}`)
          for (const pack of result.voicePacks || []) {
            if (pack.problem) lines.push(`  - ${pack.name}: ${pack.problem}`)
          }
      ...
      parameters: {
        ...
        properties: {
          ...
          onnx: {
            type: 'string',
            description:
              'add: directory holding the ONNX engine model files (the output of a Genie-TTS conversion). ' +
              'Give this instead of gpt+sovits for the ONNX engine; a pack may carry both.',
          },
          version: { type: 'string', description: 'add: model version (v2ProPlus, v4, ...). Inferred from the weights paths when omitted.' },
          notice: { type: 'string', description: 'add: a note about the material, for your own records. Defaults to the project\'s standard learning-use notice.' },
    ...
    },
    {
      name: 'tts_config',
      description:
        'Read or change dsh-say settings without editing configuration files. action=read lists every key with its ' +
